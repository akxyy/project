...
export const createDestination = (req, res) => {
  ...
  const query = 'INSERT INTO destinations (name, country, description, image_url) VALUES (?, ?, ?, ?)';
...
export const updateDestination = (req, res) => {
  const { id } = req.params;
  const { name, country, description, image_url } = req.body;
  const query = 'UPDATE destinations SET name = ?, country = ?, description = ?, image_url = ? WHERE id = ?';

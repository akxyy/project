...
export const createFlight = (req, res) => {
  ...
  const query =
    "INSERT INTO flights (destination_id, airline_name, price, departure_time, arrival_time, duration) VALUES (?, ?, ?, ?, ?, ?)";
  db.query(
    query,
    [
      destination_id,
...
export const updateFlight = (req, res) => {
  ...
  const query =
    "UPDATE flights SET destination_id = ?, airline_name = ?, price = ?, departure_time = ?, arrival_time = ?, duration = ? WHERE id = ?";
  db.query(
    query,
    [
      destination_id,
